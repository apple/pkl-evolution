...
* Add a property called `workingDirectory` to class `ExternalReader`
...
@Since { version = "0.32.0" }
function resolveForOs(enclosingUri: Uri, os: platform.OperatingSystem): EvaluatorSettings = // implementation
...
// Path starting with `\` resolves to the UNC root (analogous to drive root)
resolve(#"\\server\share\dir\sub\"#, #"\other\path"#) == #"\\server\share\other\path"#
----

**Other OS types**

There are other operating systems that have different path representations (e.g. OpenVMS, RISC OS, and AmigaOS).
These OSes are not supported by Pkl, so there is no specification for how these paths should be resolved.
...
Additionally, a new nullable property `workingDirectory` is added to `ExternalReader`.
This is added to the message passing API, and also should be added to the respective structs in downstream languages.
...
=== CLI flag for external reader workingDirectory

A new property is added to external readers, called `workingDirectory`.

This property is added to the Pkl API (to class `ExternalReader`), and also to the message passing API.

However, this property is not added as a CLI flag.
This is because the need is less likely, and it is unclear how this CLI flag should be added (how should the working dir be associated with each reader?).

If needed, it's possible to add a new CLI flag to allow this property to be set.
