...
[source,bash]
----
pkl run <root command module> [<options>]
----
...
Written file paths are printed to the command's standard error.
...
Like many other CLI flag systems, flag abbreviations may be combined (e.g. `-a -b -v -v -q some-value` is equivalent `-abvvq some-value`).

[CAUTION]
====
Flag names and short names may not conflict with link:https://pkl-lang.org/main/current/pkl-cli/index.html#common-options[common options].
...
[[_commands_as_standalone_scripts]]
...
* Force built-in options to precede the root command module URI
+
Arguments that influence Pkl evaluation (`--root-dir`, `--trace-mode`, etc.) would need to precede the command module's URI.
This would avoid ambiguity between built-in and command-provided options and avoid reducing available option names and short names for commands.
It would also avoid the potential for breaking commands when new common options are introduced.
+
This would prevent <<_commands_as_standalone_scripts,shebang invocations>> from setting evaluator options.
It forces users to learn a new rule for how to pass specific options.
Most critically, it introduces potential for misleading or dangerous ambiguities; e.g. a command may accept `--root-dir` and mislead a user into thinking it is "sandboxed" when it isn't.
...
This proposal opts to accept a reduced flag namespace and potential breaking changes to avoid exploitable ambiguities.
