...
Rules are presented here in order of more generic to less generic.
Less generic rules (bigger numbers) overrule previously defined ones, if they conflict.
...
==== [0004] Module definitions

Module, amend, and extend definitions must be written in a single line. +
...
Each import should be written in single line. +
They should be grouped in 3 groups, in this order: absolute imports, relative imports, and project imports. +
Each group should be separated by an empty line. +
Imports in the same group should not be separated by an empty line.
...
Modifiers must be written in the same line as what they are modifying, except if they go over the maximum line limit. +
...
The class header must be written in a single line if it doesn't go over the maximum line limit.
...
class ReaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaalyLongClassName extends Bar {}
----

.output
[source, pkl]
----
local open class Foo extends Bar {}

class ReaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaalyLongClassName
  extends Bar {}
----
