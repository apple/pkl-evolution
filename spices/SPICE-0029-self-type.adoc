...
Within an `open` or `abstract` class or module, `this` is the specific subclass of the receiver.

For each location in Pkl's grammar where a type may appear, the behavior of the `this` type is:
...
.11+|Class of the visibly immediate enclosing receiver
|Class method parameter type annotation
|Class method return type annotation
|Object property type annotation
|Object method parameter type annotation
|Object method return type annotation
a|Let binding type annotation

e.g. `let (val: this = ...)`
a|Function literal parameter type annotation

e.g. `(arg: this) -> ...`
|Type test/cast (`is`/`as`)
a|Explicit `new` expression

e.g. `new this { ... }`
a|Annotation type (only applicable within the bodies of `Annotation` subclasses)

e.g. `@this { ... }`

|Inside a type constraint
a|Class of the visibly immediate enclosing receiver

Does not refer to the class of the constrained type (the value of the `this` expression within a constraint).

[source,pkl]
----
class Foo {
  bar: List(first is this) // <1>
}

baz {
  qux = quux is List(first is this) // <2>
}
----
<1> `this` refers to `Foo`
<2> `this` refers to the type of `baz`

a|Object body parameter
.4+a|Class of the visibly _second_ enclosing receiver
...
outside {
  inside {
    arg: this -> // <1>
    for (val: this in something) { /* ... */ } // <1>
    when (something is this) { /* ... */ } // <1>
    [[key is this]] { /* ... */ } // <1>
  }
}
----
<1> `this` refers to the type of `outside`

|For generator binding type annotation
|When generator condition
|Inside a member predicate

a|Typealias body

e.g. `typealias Foo = this`
.2+|Disallowed
a|Class supertype

e.g. `class Foo extends this`
...
The `module` type has similar issues when used in scopes that may only access `const` values: class bodies, typealiases, annotations, and const properties/methods.
...
@Fred { plugh = true is module } // <5>
const grault = true is module // <6>

const function garply(waldo: List(first is module)) = // <7>
  true is module // <8>

class Fred extends Annotation {
  plugh: Boolean
}
----
...
<5> Use of the `module` type in an annotation body. Deprecated.
<6> Use of the `module` type in a `const` property body. Deprecated.
<7> Use of the `module` type in a `const` method parameter type constraint. Deprecated.
<8> Use of the `module` type in a `const` method body. Deprecated.
...
----
pkl: WARN: Cannot reference `module` type within an annotation body. This will be an error in a future release. (file:///path/to/file.pkl)
----
...
A few more backwards-compatible changes will be made to adopt the `this` type in existing APIs:
...
Clients of the Java API that wowk with `PType` values may need to be updated to account for `PType.THIS`.
...
As proposed, the `this` type behaves almost identically to the `module` type when used at the module level.
