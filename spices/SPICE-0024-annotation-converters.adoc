...
Multiple annotation converters can apply per property; converters for matching annotations are called in order, with the result of one being passed to the next.
...
==== Implicit converters for built-in renderers
...
New APIs for per-format property renaming will be added for each built-in renderer that has a corresponding module:
...
Each renderer will implicitly add its corresponding `<format>.Property` annotation as an annotation converter.
Manually defining an annotation converter for one of these annotation types would override the renderer's default behavior for the annotation.
